...
public class Connect4Model {
	 ...
	 * return -1. If it is a legal move, flip the color
	 *
	...
	public int placePiece(int playerID, int col) {
		int row = tryColumn(playerID, col);
	 ...
	 * @return the row of the placed piece.
	 */
	public int tryColumn(int playerID, int col){
		int row=-1;
...
//		check diagonal way
		...
		int connectNum = 0;
		for(;startx<Connect4Constant.ROW&&starty<Connect4Constant.COLUMN;startx++,starty++){
			if (currentCell == board[startx][starty]) {
				connectNum++;
				if (connectNum == 4) {
					return true;
				}
		...
		startx=row;
		starty=col;
		while(startx!=0&&starty!=Connect4Constant.COLUMN-1){
			startx--;
			starty++;
		}
		connectNum=0;
		for(;startx<Connect4Constant.ROW&&starty>-1;startx++,starty--){
			if (currentCell == board[startx][starty]) {
				connectNum++;
				if (connectNum == 4) {
					return true;
